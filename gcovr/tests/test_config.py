...
def run_cfg_test(contents, filename="test.cfg", all_options=None):
    r"""Helper to parse a config file from a string."""

    open_file = io.StringIO(contents)
    return parse_config_file(open_file, filename=filename, all_options=all_options)[0]
...
def test_option_with_boolean_values(testspec):
    ...
    options = run_cfg_test("", all_options=all_options)
    assert "target" not in options

    # if set to "no", nothing the default is explicitly set
    options = run_cfg_test("{key} = no".format(key=key), all_options=all_options)
    assert options[target] == when_no

    # if set to "yes", the value is set
    options = run_cfg_test("{key} = yes".format(key=key), all_options=all_options)
...
def test_option_choice():
    ...
    for value in (1, 3, 5):
        options = run_cfg_test(
            "testopt = {value}".format(value=value),
...
def test_nargs_optional_value():
    ...
    options = run_cfg_test("testopt = 3", all_options=all_options)
...
def test_option_that_appends():
    ...
    # when given once
    options = run_cfg_test("testopt = foo", all_options=all_options)
