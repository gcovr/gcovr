...
def argument_parser_setup(parser: ArgumentParser, default_group):
    ...
    for group_def in GCOVR_CONFIG_OPTION_GROUPS:
        if any(o.group == group_def["key"] for o in GCOVR_CONFIG_OPTIONS):
...
GCOVR_CONFIG_OPTION_GROUPS = [
    ...
    },
    {
        ...
        "description": (
            "Experimental options are for testing only and can be changed/removed in future releases."
        ),
    },
]
...
if not (getattr(sys, "frozen", False) and hasattr(sys, "_MEIPASS")):
    GCOVR_CONFIG_OPTIONS.append(
        GcovrConfigOption(
            ...
            action="store_true",
            help=(
                "Build a standalone application from the current gcovr installation. "
                "To add additional options to pyinstaller use e.g.: --bundle-app -- --help"
            ),
        )
    )
