...
def log_command(
    ...
    try:
        with capsys.disabled():
            env_str = f" (env: {str(env)})" if env else ""
            cmd_quoted = shlex.join(cmd)
            print(f"\nRunning in {cwd}{env_str}: {cmd_quoted}", file=stderr)
        yield
    finally:
        with capsys.disabled():
            print("-------------- done --------------\n", file=stderr)
...
class GcovrTestCompare:
        ...
        for pattern in output_pattern:
            for reference_file in list(sorted(self.reference_files)):
                if fnmatch.fnmatch(reference_file.name, pattern):
                    ...
                    yield self.output_dir / reference_file.name, reference_file
    ...
    def __archive_difference_data(  # pragma: no cover
        ...
        with zipfile.ZipFile(ARCHIVE_DIFFERENCES_FILE, mode="a") as fh_zip:
            fh_zip.writestr(
                (self.main_reference / reference_file.name).as_posix(),
                test_scrubbed.encode(encoding),
            )
    ...
    def run(
        ...
        *args: Union[str, Path],
        ...
        if not Path(args[0]).is_absolute():
            ...
            if (cwd / args[0]).exists():
                executable = str(cwd / args[0])
            else:
                executable = shutil.which(args[0])
                if executable is None:
                    raise RuntimeError(f"Can't resolve path {args[0]}.")
            cmd[0] = executable
        with log_command(self.capsys, cwd, env, cmd):
        ...
        if use_main:
            ...
            with chdir(cwd):
                with mock.patch.dict(os.environ, env):
                    cmd = [str(arg) for arg in args]
                    with log_command(self.capsys, cwd, env, ["gcovr-main", *cmd]):
