...
def test_config_error(gcovr_test_exec: "GcovrTestExec") -> None:
    ...
    with pytest.raises(ValueError) as exception:
        gcovr_test_exec.gcovr(
            "--config",
            "config/gcovr.error.config",
            use_main=True,
        )

    assert str(exception.value).startswith(
        "config/gcovr.error.config: 1: gcov-ignore-parse-errors: must be one of"
    )
...
def test_config_deprecated(
    ...
    gcovr_test_exec.gcovr(
        "--config",
        "config/gcovr.deprecated.config",
        use_main=True,
    )
